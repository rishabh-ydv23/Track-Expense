...
import { API_BASE_URL, getAuthToken, normalizeUserPayload } from '../config/api';
...
const ProfilePage = ({ user: userFromParent, onUpdateProfile, onLogout }) => {
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            try {
                ...
                if (data) {
                    const userData = normalizeUserPayload(data) ?? data.user ?? data;
                    setUser(userData);
    ...
    const handleSaveProfile = async () => {
        try {
            ...
            if (data) {
                const updatedUser = normalizeUserPayload(data) ?? data.user ?? data;
            ...
            // Displays API error message if available, otherwise a default message
    ...
    useEffect(() => {
        if (!userFromParent) return;
        const u = normalizeUserPayload(userFromParent) ?? userFromParent;
        setUser((prev) => ({ ...prev, ...u }));
        setTempUser((prev) => ({ ...prev, ...u }));
    }, [userFromParent]);
